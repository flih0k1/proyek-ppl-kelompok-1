                            ...
                            <?php foreach ($getdata as $row): ?>
                                ...
                                <?php echo badge($row->mt_status) ?></td>
                                    ...
                                    <td class="text-center pe-3">
                                            <?php if ($row->mt_status == 'pending'): ?>
                                                <button class="btn btn-sm btn-success mb-1" title="Konfirmasi" onclick="updateStatus('<?= $row->mt_code ?>', 'process')">
                                                ...
                                                <?php elseif ($row->mt_status == 'process'): ?>
                                                    <button class="btn btn-sm btn-primary mb-1" title="Selesaikan" onclick="updateStatus('<?= $row->mt_code ?>', 'selesai')">
...
<script>
    ...
    function updateStatus(code, status) {
        Swal.fire({
            ...
            if (res.value) {
                $.ajax({
                        url: '<?php echo site_url('pimpinan/postdata/maintenance/update_maintenance') ?>',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            code: code,
    ...
    function deletemt(code) {
        Swal.fire({
            ...
            if (res.value) {
                $.ajax({
                        url: '<?php echo site_url('pimpinan/postdata/maintenance/delete_maintenance') ?>',
                        type: 'POST',
                        dataType: 'json',
                        data: {
                            code: code,
