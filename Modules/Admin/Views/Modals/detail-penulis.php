...
$penulis = $this->db->table('tb_penulis')->where('penulis_id', $id)->get()->getRow();

if (!$penulis): ?>
    <div class="text-center py-4 text-muted">
        <i class="bi bi-exclamation-circle fs-1"></i>
        <p class="mt-2">Penulis tidak ditemukan.</p>
    </div>
<?php return; endif;

$buku_list = $this->db->table('tb_buku')
    ->where('buku_penulis', $penulis->penulis_nama)
...
<div class="d-flex align-items-center gap-3 mb-3">
    ...
    <div>
        <h6 class="fw-bold mb-0"><?= esc($penulis->penulis_nama) ?></h6>
        <small class="text-muted"><?= $total ?> buku ditulis oleh penulis ini</small>
    </div>
</div>
...
<?php if (empty($buku_list)): ?>
    <div class="text-center text-muted py-4">
        ...
        Belum ada buku oleh penulis ini.
    </div>
<?php else: ?>
