...
$kategori = $this->db->table('tb_kategori_buku')->where('kategori_id', $id)->get()->getRow();

if (!$kategori): ?>
    <div class="text-center py-4 text-muted">
        <i class="bi bi-exclamation-circle fs-1"></i>
        <p class="mt-2">Kategori tidak ditemukan.</p>
    </div>
<?php return; endif;

$buku_list = $this->db->table('tb_buku')
    ->where('buku_kategori_id', $id)
...
<div class="d-flex align-items-center gap-3 mb-3">
    ...
    <div>
        <h6 class="fw-bold mb-0"><?= esc($kategori->kategori_nama) ?></h6>
        <small class="text-muted"><?= $total ?> buku dalam kategori ini</small>
    </div>
</div>
