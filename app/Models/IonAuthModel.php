...
class IonAuthModel
{
	...
	public function activate($id, string $code=''): bool
	{
		...
		if (!$code || ($user && $user->id == $id))
		{
			...
			$this->db->table($this->tables['tb_users'])->update($data, ['id' => $id]);
	...
	public function clearForgottenPasswordCode(string $identity): bool
	{
		...
		return $this->db->table($this->tables['tb_users'])->where($this->identityColumn, $identity)->update($data);
	}
	...
	public function changePassword(string $identity, string $old, string $new): bool
	{
		...
		$builder = $this->db->table($this->tables['tb_users']);
		$query   = $builder
					   ->select('id, password')
	...
	public function usernameCheck(string $username): bool
	{
		...
		return $this->db->table($this->tables['tb_users'])
	...
	public function forgottenPassword(string $identity)
	{
		...
		$this->triggerEvents('extra_where');
		$this->db->table($this->tables['tb_users'])->update($update, [$this->identityColumn => $identity]);
	...
	public function register(string $identity, string $password, string $email, array $additionalData=[], array $groups=[])
	{
		...
		$query = $this->db->table($this->tables['tb_groups'])->where(['name' => $this->config->defaultGroup], 1)->get()->getRow();
		...
		$userData = array_merge($this->filterData($this->tables['tb_users'], $additionalData), $data);

		$this->triggerEvents('extra_set');

		$this->db->table($this->tables['tb_users'])->insert($userData);

		$id = $this->db->insertId($this->tables['tb_users'] . '_id_seq');
	...
	public function login(string $identity, string $password, bool $remember=false): bool
	{
		...
		$this->triggerEvents('extra_where');
		$query = $this->db->table($this->tables['tb_users'])
	...
	public function getAttemptsNum(string $identity, $ipAddress=null): int
	{
		if ($this->config->trackLoginAttempts)
		{
			$builder = $this->db->table($this->tables['tb_login_attempts']);
	...
	public function users($groups=null): self
	{
		...
		if (isset($groups))
		{
			...
			if (! empty($groupNames))
			{
				$builder->join($this->tables['tb_groups'], $this->tables['tb_users_groups'] . '.' . $this->join['tb_groups'] . ' = ' . $this->tables['tb_groups'] . '.id', 'inner');
				$builder->whereIn($this->tables['tb_groups'] . '.name', $groupNames);
			}
			if (! empty($groupIds))
			{
				$builder->{$orWhereIn}($this->tables['tb_users_groups'] . '.' . $this->join['tb_groups'], $groupIds);
			}
		}
	...
	public function user(int $id=0): self
	{
		...
		$this->limit(1);
		$this->orderBy($this->tables['tb_users'] . '.id', 'desc');
	...
	public function getUsersGroups(int $id=0)
	{
		...
		$builder = $this->db->table($this->tables['tb_users_groups']);
		return $builder->select($this->tables['tb_users_groups'] . '.' . $this->join['tb_groups'] . ' as id, ' . $this->tables['tb_groups'] . '.name, ' . $this->tables['tb_groups'] . '.description')
	...
	public function addToGroup($groupIds, int $userId=0): int
	{
		...
		foreach ($groupIds as $groupId)
		{
			...
			if ($this->db->table($this->tables['tb_users_groups'])->insert([
																	$this->join['tb_groups'] => (float)$groupId,
																	$this->join['tb_users']  => (float)$userId  ]))
			{
	...
	public function removeFromGroup($groupIds=0, int $userId=0): bool
	{
		...
		if (! empty($groupIds))
		{
			...
			foreach ($groupIds as $groupId)
			{
				$builder->delete([$this->join['tb_groups'] => (int)$groupId, $this->join['tb_users'] => $userId]);
		...
		{
			if ($return = $builder->delete([$this->join['tb_users'] => $userId]))
			{
	...
	{
		$this->triggerEvents('tb_groups');

		$builder = $this->db->table($this->tables['tb_groups']);
	...
	public function update(int $id, array $data): bool
	{
		...
		$data = $this->filterData($this->tables['tb_users'], $data);
		...
		$this->triggerEvents('extra_where');
		$this->db->table($this->tables['tb_users'])->update($data, ['id' => $user->id]);
	...
	public function deleteUser(int $id): bool
	{
		...
		$this->db->table($this->tables['tb_users'])->delete(['id' => $id]);
	...
	public function updateLastLogin(int $id): bool
	{
		...
		$this->db->table($this->tables['tb_users'])->update(['last_login' => time()], ['id' => $id]);
	...
	public function rememberUser(string $identity): bool
	{
		...
		{
			$this->db->table($this->tables['tb_users'])->update(['remember_selector' => $token->selector,
	...
	public function createGroup(string $groupName='', string $groupDescription='', array $additionalData=[])
	{
		...
		$existingGroup = $this->db->table($this->tables['tb_groups'])->where(['name' => $groupName])->countAllResults();
		if ($existingGroup !== 0)
		{
		...
		{
			$data = array_merge($this->filterData($this->tables['tb_groups'], $additionalData), $data);
		}

		$this->triggerEvents('extra_group_set');

		// insert the new group
		$this->db->table($this->tables['tb_groups'])->insert($data);
		$groupId = $this->db->insertId($this->tables['tb_groups'] . '_id_seq');
	...
	public function updateGroup(int $groupId, string $groupName='', array $additionalData=[]): bool
	{
		...
		if (! empty($groupName))
		{
			...
			// bail if the group name already exists
			$existingGroup = $this->db->table($this->tables['tb_groups'])->getWhere(['name' => $groupName])->getRow();
		...
		$group = $this->db->table($this->tables['tb_groups'])->getWhere(['id' => $groupId])->getRow();
		...
		$this->db->table($this->tables['tb_groups'])->update($data, ['id' => $groupId]);
	...
	public function deleteGroup(int $groupId): bool
	{
		...
		$this->db->table($this->tables['tb_users_groups'])->delete([$this->join['tb_groups'] => $groupId]);
		// remove the group itself
		$this->db->table($this->tables['tb_groups'])->delete(['id' => $groupId]);
	...
	protected function setPasswordDb(string $identity, string $password): bool
	{
		...
		$this->db->table($this->tables['tb_users'])->update($data, [$this->identityColumn => $identity]);
